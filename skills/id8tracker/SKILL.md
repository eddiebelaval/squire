...
|-----------------|-------------|---------------------|
| id8scout (BUILD) | "Scout: Validation complete - BUILD verdict" | VALIDATING → VALIDATED |
| id8scout (KILL) | "Scout: Validation complete - KILL verdict" | → KILLED |
| id8architect | "Architect: Architecture doc complete" | VALIDATED → ARCHITECTING |
| id8launch | "Launch: Product launched to {channel}" | BUILDING → LAUNCHING |
| id8growth | "Growth: Experiment {name} completed" | (no transition, activity log) |
| id8ops | "Ops: SOP created for {process}" | GROWING → OPERATING |
| id8exit | "Exit: Exit memo drafted" | OPERATING → EXITING |
